...
class TestInitServers:
    ...
    def test_init_servers_skips_existing_servers(self, mock_db_session):
        ...
        servers_config = [
            {"id": "existing1", "name": "Existing Server 1", "ip": "127.0.0.1", "port": 25565}
        ]
        
        # Mock that server already exists
        existing_server = Mock()
        mock_db_session.query.return_value.filter.return_value.first.return_value = existing_server
    ...
    def test_init_servers_mixed_new_and_existing(self, mock_db_session):
        ...
        servers_config = [
            {"id": "existing", "name": "Existing Server", "ip": "127.0.0.1", "port": 25565},
...
class TestSaveResults:
    ...
    def test_save_results_successful_entries_only(self, mock_db_session):
        ...
        results = [
            {'server_id': 'server1', 'player_count': 3, 'players': ['Player1'], 'success': True},
            {'server_id': 'server2', 'player_count': 0, 'players': [], 'success': False},  # Should skip
            {'server_id': 'server3', 'player_count': 2, 'players': ['Player2'], 'success': True}
        ]
    ...
    def test_save_results_reuses_existing_players(self, mock_db_session):
        ...
        results = [
            {'server_id': 'server1', 'player_count': 2, 'players': ['ExistingPlayer', 'NewPlayer'], 'success': True}
        ]
        ...
        mock_db_session.query.return_value.filter.return_value.first.side_effect = [existing_player, None]
    ...
    def test_save_results_associates_players_with_snapshot(self, mock_db_session):
        ...
        results = [
            {'server_id': 'server1', 'player_count': 1, 'players': ['TestPlayer'], 'success': True}
        ]
    ...
    def test_save_results_with_no_players(self, mock_db_session):
        ...
        with patch('scraper.PlayerCount', return_value=mock_player_count) as mock_pc_class:
            with patch('scraper.Player') as mock_player_class:
                save_results(mock_db_session, results)
    ...
    def test_save_results_timestamp_consistency(self, mock_db_session):
        ...
        with patch('scraper.PlayerCount', side_effect=[mock_player_count1, mock_player_count2]) as mock_pc_class:
            with patch('scraper.datetime') as mock_datetime:
...
class TestDatabaseErrorHandling:
    ...
    def test_save_results_database_error_on_flush(self, mock_db_session):
        ...
        results = [
            {'server_id': 'server1', 'player_count': 1, 'players': ['Player1'], 'success': True}
        ]
...
class TestPlayerHandling:
    ...
    def test_save_results_duplicate_players_in_result(self, mock_db_session):
        ...
        results = [
            {'server_id': 'server1', 'player_count': 2, 'players': ['Player1', 'Player1'], 'success': True}
        ]
