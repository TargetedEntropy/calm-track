...
def sample_player_counts(db_session, sample_servers):
    ...
    for i in range(30):
        ...
        for j, server in enumerate(sample_servers):
            ...
            pc = PlayerCount(
                server_id=server.id, timestamp=timestamp, player_count=count
            )
...
class TestGraphGeneration:
    ...
    def test_generate_graph_different_periods(
        self, sample_player_counts, smart_mock_templates
    ):
...
class TestHelperFunctions:
    ...
    def test_create_plot(
        self, mock_close, mock_savefig, sample_servers, sample_player_counts, db_session
    ):
        """Test plot creation"""
        server = get_server_by_id("test1", db_session)
    ...
    def test_generate_html_content(
        self, sample_servers, sample_player_counts, db_session, smart_mock_templates
    ):
...
class TestEdgeCases:
    ...
    def test_graph_generation_with_single_data_point(
        self, sample_servers, db_session, smart_mock_templates
    ):
        """Test graph generation with only one data point"""
        # Add a single player count
        pc = PlayerCount(server_id="test1", timestamp=datetime.utcnow(), player_count=5)
        db_session.add(pc)
        db_session.commit()
    ...
    def test_large_period_with_limited_data(
        self, sample_player_counts, smart_mock_templates
    ):
...
class TestDataIntegrity:
    ...
    def test_server_player_count_relationship(self, sample_servers, db_session):
        ...
        pc = PlayerCount(
            server_id=server.id, timestamp=datetime.utcnow(), player_count=10
        )
        db_session.add(pc)
        db_session.commit()
    ...
    def test_player_snapshot_association(
        self, sample_servers, sample_players, db_session
    ):
        ...
        pc = PlayerCount(
            server_id=server.id, timestamp=datetime.utcnow(), player_count=1
        )
